...
class topology:
            ...
            if block[0].find('dihedrals') >= 0:
                for i in range(len(block)): #lines
                    if block[i][0]==';' or block[i][0]=='[':
                        ...
                        if int(top[4])==9:
                            try:
                                self.dihedrals_list.append(self.dihedrals(top[0],top[1],top[2],top[3],top[4],top[5],top[6],top[7]))
                                #print top[0],top[1],top[2],top[3],top[4],top[5],top[6],top[7]
                            except Exception:
                                print 'err in',top
                                self.dihedrals_list.append(self.dihedrals(top[0],top[1],top[2],top[3],top[4]))
                        if int(top[4])==4:
                            self.impropers_list.append(self.impropers(top[0],top[1],top[2],top[3],top[4],top[5],top[6],top[7]))
    ...
    def write_top(self,filename):
        ...
        for i in self.bonds_list:print >> f,i.cprint();
        print >> f, ''    
        #write [ pairs ]
        ...
        print >>f, '[ dihedrals ]; prop'
        for i in self.dihedrals_list:print >> f,i.cprint();
        print >> f, ''               
        #write [ impropers ]
        print >>f, '[ dihedrals ]; improp'
    ...
    class dihedrals:
        ...
        def __init__(self,i,j,k,l,funct,phase='',kd='',pn=''):
            ...
            try:
                ...
                self.pn      = ''
        def cprint(self):
            return s(self.i,5) + s(self.j,5) + s(self.k,5) + s(self.l,6)+ s(self.funct,6) + s(self.phase,12) + s(self.kd,12) + s(self.pn,12) 
